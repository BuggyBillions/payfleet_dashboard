...
const ViewCompanyModal: React.FC<ViewCompanyModalProps> = ({
  ...
  return (
    <Modal onClose={onClose}>
      ...
      <div className="bg-secondary border border-primary/10 rounded-2xl p-4 flex flex-col sm:flex-row items-center gap-4">
        ...
        <div className="flex-1 text-center sm:text-left space-y-1">
          <div className="flex items-center justify-center sm:justify-between flex-wrap gap-2">
            ...
            <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-lg text-primary text-xs font-bold font-mono">
              <LuWallet size={14} /> {formatterUtility(compBalance)}
            </span>
          </div>
      ...
      {/* Navigation Tabs */}
      <div className="flex items-center gap-2 my-2 border-b border-primary/10 pb-2 overflow-x-auto w-full">
        <button
          type="button"
          onClick={() => setActiveTab("overview")}
          className={`px-3.5 py-1.5 rounded-lg text-xs font-semibold transition cursor-pointer ${activeTab === "overview"
        ...
        <button
          ...
          className={`px-3.5 py-1.5 rounded-lg text-xs font-semibold transition cursor-pointer flex items-center gap-1.5 ${activeTab === "staff"
              ? "bg-primary text-white shadow-xs"
              : "text-textBlack/70 hover:text-textBlack hover:bg-secondary"
            }`}
        >
          <span className="flex ">Staff</span>
        </button>
        <button
          type="button"
          onClick={() => setActiveTab("documents")}
          className={`px-3.5 py-1.5 rounded-lg text-xs font-semibold transition cursor-pointer flex items-center gap-1.5 ${activeTab === "documents"
              ? "bg-primary text-white shadow-xs"
              : "text-textBlack/70 hover:text-textBlack hover:bg-secondary"
            }`}
        >
        ...
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3.5">
            {documentsList.map((doc, idx) => (
              <div
                key={idx}
                className="p-4 rounded-xl bg-secondary border border-primary/10 flex md:flex-col items-center md:items-start md:justify-between gap-3"
              >
